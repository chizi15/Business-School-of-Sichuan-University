...
account_1 = acct_code_sm_loss_cost[
    acct_code_sm_loss_cost["busdate"].between(first_day, last_day)
]
...
acct_code_sm_loss_cost = acct_code_sm_loss_cost[
    acct_code_sm_loss_cost["code"].isin(acct_grup_1["code"].unique())
]
print(
    f'去除空值与非正毛利率的样本后，有{acct_code_sm_loss_cost["code"].nunique()}个codes',
    "\n",
)
...
acct_code_sm_loss_cost = acct_code_sm_loss_cost[
    acct_code_sm_loss_cost["code"].isin(acct_grup["code"].unique())
]
...
acct_code_sm_loss_cost["amount"] = acct_code_sm_loss_cost["amount"].apply(
    lambda x: np.log1p(x)
)
acct_code_sm_loss_cost["price"] = acct_code_sm_loss_cost["price"].apply(
    lambda x: np.log1p(x)
)
# 筛选销量与价格负相关性强的单品
typeA = []
...
list_df = [
    acct_code_sm_loss_cost[acct_code_sm_loss_cost["name"].isin(group)]
...
acct_code_sm_loss_cost["amount"] = acct_code_sm_loss_cost["amount"] / (
    1 - acct_code_sm_loss_cost["loss_rate"] / 100
)
acct_code_sm_loss_cost["sum_cost"] = (
    acct_code_sm_loss_cost["amount"] * acct_code_sm_loss_cost["unit_cost"]
)
acct_code_sm_loss_cost["sum_price"] = (
    acct_code_sm_loss_cost["amount"] * acct_code_sm_loss_cost["price"]
)
acct_code_sm_loss_cost["profit"] = (
    acct_code_sm_loss_cost["sum_price"] - acct_code_sm_loss_cost["sum_cost"]
) / acct_code_sm_loss_cost["sum_price"]
...
if (
    abs(
        np.mean(
            acct_code_sm_loss_cost["sum_cost"] / acct_code_sm_loss_cost["amount"]
            - acct_code_sm_loss_cost["unit_cost"]
        )
    )
    < 1e-3
):
...
for i in acct_code_sm_loss_cost["name"].unique():
    ...
    acct_ori["profit"] = (acct_ori["sum_price"] - acct_ori["sum_cost"]) / acct_ori[
        "sum_price"
    ]
