...
if not isinstance(commodity["单品编码"].iloc[0], str):
    commodity[["单品编码", "小分类编码", "中分类编码", "大分类编码"]] = (
        commodity[["单品编码", "小分类编码", "中分类编码", "大分类编码"]]
        .astype("Int64")
        .astype(str)
    )
...
if add_order_loss:
    ...
    account_order = pd.merge(
        account, order, on=["课别", "单品编码", "日期"], how="left"
    )
    account_order = pd.merge(
        account_order, commodity, on=["课别", "单品编码"], how="left"
    )
    ...
    random_loss_real = np.random.choice(
        division[:-1], size=len(account_order), p=count / len(loss_real)
    )
    # 将两个直方图放在一张图上进行对比，并输出legend
    ax = sns.histplot(loss_real, bins=100, kde=True, alpha=0.2, label="原始损耗率分布")
    ax = sns.histplot(
        random_loss_real, bins=100, kde=True, alpha=0.2, label="扩充后的随机损耗率分布"
    )
    ax.legend()
    ...
    account_order["实际订货量"] = account_order["amount"] * (
        1 + account_order["损耗率(%)"] / 100
    )

    if not isinstance(account_order["小分类编码"].iloc[0], str):
        account_order[["单品名称", "小分类编码", "中分类编码", "大分类编码"]] = (
            account_order[["单品名称", "小分类编码", "中分类编码", "大分类编码"]]
            .astype("Int64")
            .astype(str)
        )
    ...
    order_com = pd.merge(
        order, commodity, on=["课别", "单品编码", "单品名称"], how="right"
    )

    if not isinstance(order_com["小分类编码"].iloc[0], str):
        order_com[["单品名称", "小分类编码", "中分类编码", "大分类编码"]] = (
            order_com[["单品名称", "小分类编码", "中分类编码", "大分类编码"]]
            .astype("Int64")
            .astype(str)
        )
    # 将order_com['损耗率(%)']中>100的值替换为100, <0的值替换为0
    order_com["损耗率(%)"] = order_com["损耗率(%)"].apply(
        lambda x: 100 if x > 100 else x
    )
    order_com["损耗率(%)"] = order_com["损耗率(%)"].apply(lambda x: 0 if x < 0 else x)
    # 计算order_com中各个单品和小分类的中位数损耗率，并忽略掉空值
    loss_real_code = (
        order_com.groupby(["单品编码"])["损耗率(%)"]
        .apply(lambda x: x.mean(skipna=True))
        .reset_index()
        .rename(columns={"损耗率(%)": "平均损耗率(%)_单品编码"})
    )
    loss_real_sm = (
        order_com.groupby(["小分类编码"])["损耗率(%)"]
        .apply(lambda x: x.mean(skipna=True))
        .reset_index()
        .rename(columns={"损耗率(%)": "平均损耗率(%)_小分类编码"})
    )
    # 填充loss_real_median中的损耗率空值
    loss_real_code["平均损耗率(%)_单品编码"].fillna(
        loss_real_code["平均损耗率(%)_单品编码"].mean(), inplace=True
    )
    loss_real_sm["平均损耗率(%)_小分类编码"].fillna(
        loss_real_sm["平均损耗率(%)_小分类编码"].mean(), inplace=True
    )
    ...
    order_com["平均损耗率(%)_小分类编码"] = round(
        order_com["平均损耗率(%)_小分类编码"], 2
    )
    ...
    loss_real_sm_diff = (
        order_com.groupby(["小分类编码", "小分类名称"])["平均损耗率(%)_小分类编码"]
        .apply(lambda x: x.unique()[0])
        .reset_index()
        .rename(columns={"平均损耗率(%)_小分类编码": "平均损耗率(%)_小分类编码_不同值"})
    )
    loss_real_code_diff = (
        order_com.groupby(["单品编码", "单品名称"])["平均损耗率(%)_单品编码"]
        .apply(lambda x: x.unique()[0])
        .reset_index()
        .rename(columns={"平均损耗率(%)_单品编码": "平均损耗率(%)_单品编码_不同值"})
    )
    # 按损耗率降序排列
    loss_real_sm_diff.sort_values(
        by=["平均损耗率(%)_小分类编码_不同值"], ascending=False, inplace=True
    )
    loss_real_code_diff.sort_values(
        by=["平均损耗率(%)_单品编码_不同值"], ascending=False, inplace=True
    )
    ...
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.lower()
    )
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.replace("hlj", "")
    )
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.replace("好邻居", "")
    )
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.replace("悦活里", "")
    )
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.replace("悦活荟", "")
    )
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.replace("悦丰硕", "")
    )
    loss_real_code_diff["单品名称"] = loss_real_code_diff["单品名称"].apply(
        lambda x: x.replace("悦令鲜", "")
    )

    loss_real_sm_diff["小分类名称"] = loss_real_sm_diff["小分类名称"].apply(
        lambda x: x.replace("好邻居", "")
    )
    loss_real_sm_diff["小分类名称"] = loss_real_sm_diff["小分类名称"].apply(
        lambda x: x.replace("悦活里", "")
    ...
    with pd.ExcelWriter(f"{output_path}/损耗率.xlsx") as writer:
        loss_real_sm_diff.to_excel(
            writer, index=False, sheet_name="平均损耗率(%)_小分类编码_不同值"
        )
        loss_real_code_diff.to_excel(
            writer, index=False, sheet_name="平均损耗率(%)_单品编码_不同值"
        )
