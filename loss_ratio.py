...
if add_order_loss:
    ...
    order_com = pd.merge(order, commodity, on=['课别', '单品编码', '单品名称'], how='left')
    ...
    loss_real_code = order_com.groupby(['单品编码'])['损耗率(%)'].apply(lambda x: x.median(skipna=True)).reset_index().rename(columns={'损耗率(%)': '平均损耗率(%)_单品编码'})
    loss_real_sm = order_com.groupby(['小分类编码'])['损耗率(%)'].apply(lambda x: x.median(skipna=True)).reset_index().rename(columns={'损耗率(%)': '平均损耗率(%)_小分类编码'})
