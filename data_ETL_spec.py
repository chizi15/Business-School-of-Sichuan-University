...
match organ:
    case 'DH':
        ...
        prediction = pd.read_csv(f"D:\Work info\WestUnion\data\origin\\{organ}\\prediction.csv",
                                 dtype={'code': str}, parse_dates=['busdate'], infer_datetime_format=True)
    ...
    case _:
        ...
        prediction = pd.read_csv(f"D:\Work info\WestUnion\data\origin\\{organ}\\fresh-forecast-order.csv",
                                 ...
                                 parse_dates=['busdate'], infer_datetime_format=True)
        promotion = pd.read_csv(f"D:\Work info\WestUnion\data\origin\\{organ}\\promotion.csv",
                                parse_dates=['busdate'], infer_datetime_format=True, dtype={'code': str})
        ...
        match sort:
            ...
            case '绿叶类':
                ...
                abnormal_day = abnormal_day[0]
            case _:
                comodt_seg = commodity[commodity['sm_sort_name'].str.contains(f'{sort}')]
                comodt_seg = comodt_seg[~((comodt_seg['name'] == '贵妃西红柿') | (comodt_seg['name'] == '岭上水果番茄') \
                             | (comodt_seg['name'] == '草莓番茄') | (comodt_seg['name'] == '奶油西红柿') \
                             | (comodt_seg['name'] == '巧克力西红柿'))]
                # 剔除异常日期
                critical_day = datetime.datetime(2022, 2, 4)
                abnormal_day = pd.DataFrame([datetime.datetime(2022, 8, 18) + pd.to_timedelta(np.arange(9), unit='d')])
                abnormal_day = pd.concat([abnormal_day, pd.DataFrame([datetime.datetime(2022, 10, 21)])], axis=1)
