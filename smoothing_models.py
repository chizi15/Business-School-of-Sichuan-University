...
def simple(x, fc, alpha=None):
    ...
    return {'predict': Y[-fc:], 'alpha': alpha, 'rmse': rmse_index, 'aic': aic_index, 'std_residual': std_residual}
...
def linear(x, fc, alpha=None, beta=None):
    ...
    return {'predict': Y[-fc:], 'alpha': alpha, 'beta': beta, 'rmse': rmse_index, 'aic': aic_index, 'std_residual': std_residual}
...
def double_mul(x, fc, alpha=None, beta=None):
    ...
    if alpha is None or beta is None:
        ...
        print('ols_loss_func', para_ols_slsqp.x)
        print('para_differential_evolution', para_differential_evolution.x)
        alpha, beta = para_differential_evolution.x
...
def additive(x, m, fc, alpha=None, beta=None, gamma=None):
    ...
    if alpha is None or beta is None or gamma is None:
        ...
        initial_values = np.array([0.9, 0.9, 0.9])
        boundaries = [(1/2*1e-1, 1-1/2*1e-1), (1/2*1e-1, 1-1/2*1e-1), (1/2+1e-1, 1-1/2*1e-1)]
        model = 'additive'
        ...
        print('ols_loss_func', para_ols_slsqp.x)
        print('para_differential_evolution', para_differential_evolution.x)
        alpha, beta, gamma = para_ols_slsqp.x
...
def multiplicative(x, m, fc, alpha=None, beta=None, gamma=None):
    ...
    if alpha is None or beta is None or gamma is None:
        ...
        initial_values = np.array([0.9, 0.9, 2/3])
        boundaries = [(1/2*1e-1, 1-1/2*1e-1), (1/2*1e-1, 1-1/2*1e-1), (1/2+1e-1, 1-1/2*1e-1)]
...
def multiseasonal_add_day(x, m_year=365, m_quarter=round(365/4), m_month=round(365/12), m_week=7, fc=4*1,
    ...
    if alpha is None or beta is None or gamma_year is None or gamma_quarter is None or gamma_month is None or gamma_week is None:
        ...
        initial_values = np.array([.9] * 6)
        boundaries = [(1e-2, 1-1e-2)] * len(initial_values)
    ...
    return {'len(s_year)': len(s_year), 'max(s_year)': max(s_year), 'min(s_year)': min(s_year),
            ...
            'res': rmse_index - std_residual, 'predict': Y[-fc:], 'alpha': alpha, 'beta': beta,
...
def multiseasonal_mul_day(x, m_year=365, m_quarter=round(365/4), m_month=round(365/12), m_week=7, fc=4*1,
                          alpha=None, beta=None, gamma_year=None, gamma_quarter=None, gamma_month=None, gamma_week=None):
    """
...
if __name__ == "__main__":
    ...
    g3 = multiseasonal_add_day(day)
    print('multiseasonal_add_day', g3, '\n')
    g4 = multiseasonal_add_week(week)
    print('multiseasonal_add_week', g4, '\n')
    g1 = multiseasonal_mul_day(day)
    print('multiseasonal_mul_day', g1, '\n')
    g2 = multiseasonal_mul_week(week)
    print('multiseasonal_mul_week', g2)
