...
# question_1
# 获取茄类在训练集上的样本
sm_qielei = account_commodity_mean[account_commodity_mean['sm_sort_name'] == "茄类"][:-periods]
sm_qielei.to_excel(r"D:\Work info\SCU\MathModeling\2023\data\processed\question_1_2\sm_qielei.xlsx", index=False)

# 以'busdate'为横坐标，'amount'为纵坐标，画出时序图。用sns来画图，使图更美观，使横坐标的日期不会重叠，并且横坐标以每月为时间间隔显示
# sm_qielei['busdate'] = pd.to_datetime(sm_qielei['busdate'])
# sm_qielei = sm_qielei.sort_values(by='busdate')
...
fig2.savefig(r"D:\Work info\SCU\MathModeling\2023\data\processed\question_1_2\fit_components.svg", dpi=300, bbox_inches='tight')
...
qielei_prophet_amount.to_excel(r"D:\Work info\SCU\MathModeling\2023\data\processed\question_1_2\qielei_prophet_amount_with_effect.xlsx", index=False)
...
sm_qielei_amt_ext = extendSample(sm_qielei['amt_no_effect'].values, max_weight=int(len(sm_qielei['amt_no_effect'].values)**(extend_power)))
...
f.fit()
q_steady = st.gamma.ppf(profit_avg, *f.fitted_param['gamma'])
...
all_set.to_excel(r"D:\Work info\SCU\MathModeling\2023\data\processed\question_1_2\qielei_all_set.xlsx", index=False, encoding='utf-8-sig')
...
f_star.fit()
q_steady_star = st.gamma.ppf(profit_star, *f_star.fitted_param['gamma'])
