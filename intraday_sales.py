...
weekday_dict = {1: '星期一', 2: '星期二', 3: '星期三', 4: '星期四', 5: '星期五', 6: '星期六', 7: '星期日'}
...
output_path = [os.path.join(base_path, sm_sort_name[i] + '\\') for i in range(len(sm_sort_name))]
...
run_code_seg.drop(columns=['销售单价(元/千克)', '销售类型', '是否打折销售'], inplace=True)
run_code_seg.rename(columns={'销售日期': 'busdate', '扫码销售时间': 'selltime', '单品编码': 'code', '销量(千克)': 'amount'}, inplace=True)
...
for i in range(len(sm_sort_name)):
    ...
    run_seg['selltime'] = pd.to_datetime(run_seg['selltime'], infer_datetime_format=True).dt.strftime('%H:%M:%S')
    run_seg['selltime'] = run_seg['selltime'].apply(lambda x: parse(x).time())
    print(run_seg.dtypes, '\n')

    # 在run_seg中添加一列，用于标记星期
    run_seg['weekday'] = run_seg['busdate'].apply(lambda x: weekday_dict[x.weekday() + 1])
    ...
    assert (run_seg[(run_seg['is_holiday'] == True) & (run_seg['is_workday'] == True)].shape[0]) == 0, 'is_holiday和is_workday不是取反的关系'
    # 去掉节假日的样本
    run_seg = run_seg[run_seg['is_holiday'] == False]
    run_seg_weekday = run_seg.groupby(['weekday', 'selltime'])['amount'].mean().reset_index()
    ...
    run_seg_weekday_resampled = run_seg_weekday.groupby('weekday').resample('15T', on='selltime').sum(numeric_only=True).reset_index()
    # 将 'selltime' 列转换为 str 类型，且只保留时分
    run_seg_weekday_resampled['selltime'] = run_seg_weekday_resampled['selltime'].dt.strftime('%H:%M')
    # 按weekday列进行排序取值不同拆分成不同的dataframe
    run_seg_weekday_resampled_dict = dict(tuple(run_seg_weekday_resampled.groupby('weekday')))
    ...
    for j, (key, df) in enumerate(run_seg_weekday_resampled_dict.items()):
        ...
        plt.tight_layout()
        plt.savefig(f"{output_path[i]}" + f"{sm_sort_name[i]}_{key}_日内平均销量曲线.svg", format='svg')
        plt.show()
