...
running = pd.read_csv(
    "D:\Work info\WestUnion\data\origin\DH\\running.csv",
...
acct_pred_com_stk = pd.merge(
    acct_pred_com, stock, how="left", on=["organ", "code", "busdate"]
)
...
running_grup.rename(
    columns={
        "selldate": "busdate",
        "amount": "amount_run",
        "sum_sell": "sum_price_run",
    },
    inplace=True,
)

# export to sheet
acct_pred_com_stk_run = pd.merge(
    acct_pred_com_stk, running_grup, how="left", on=["organ", "code", "busdate"]
)
match chosen:
    case "origin":
        acct_pred_com_stk_run.to_csv(
            f"D:\Work info\WestUnion\data\processed\DH\\acct_pred_com_stk_run_{chosen}.csv",
            encoding="utf_8_sig",
            index=False,
        )
    case "drop_fcst_na":
    ...
    case "exact":
        acct_pred_com_stk_run = acct_pred_com_stk_run[
            (acct_pred_com_stk_run["amount_run"] == acct_pred_com_stk_run["amount"])
            & (
                acct_pred_com_stk_run["sum_price_run"]
                == acct_pred_com_stk_run["sum_price"]
            )
        ]
        acct_pred_com_stk_run.to_csv(
            f"D:\Work info\WestUnion\data\processed\DH\\acct_pred_com_stk_run_{chosen}.csv",
    ...
    case "exact_dropna":
        ...
        acct_pred_com_stk_run.dropna(
            subset=["fcstamou", "amount", "amount_run", "sum_price_run"], inplace=True
        )
        acct_pred_com_stk_run.to_csv(
            f"D:\Work info\WestUnion\data\processed\DH\\acct_pred_com_stk_run_{chosen}.csv",
            encoding="utf_8_sig",
...
if tableau == 1:
    ...
    for _ in range(len(acct_pred_com_stk_run.groupby("organ"))):
        acct_pred_com_stk_run_org = acct_pred_com_stk_run[
            acct_pred_com_stk_run["organ"]
            == acct_pred_com_stk_run.groupby("organ", as_index=False).size()["organ"][_]
        ]
        acct_pred_com_stk_run_org.to_csv(
            f"D:\Work info\WestUnion\data\processed\DH\organ\\"
        ...
        if (
            len(acct_pred_com_stk_run_org)
            != acct_pred_com_stk_run.groupby("organ", as_index=True).size().values[_]
        ):
