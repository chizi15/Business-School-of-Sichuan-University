...
def ordering_pricing(sm_qielei_all, _):
    ...
    sm_qielei_all = sm_qielei_all.append(
        [sm_qielei_all.iloc[-1, :]] * (periods - 1), ignore_index=True
    )
    sm_qielei_all["busdate"][-(periods - 1) :] = sm_qielei_all["busdate"][
        -(periods - 1) :
    ] + pd.to_timedelta(np.arange(1, periods), unit="d")
    sm_qielei_all[-(periods - 1) :]["amount"] += np.random.normal(
        0,
        max(
            0,
            min(
                sm_qielei_all["amount"].std() ** 0.5,
                sm_qielei_all["amount"].mean() / 10,
            ),
        ),
        periods - 1,
    ...
    sm_qielei_all.drop(
        columns=["amount_y", "sum_cost_y", "sum_price_y", "unit_cost_y", "loss_rate_y"],
        inplace=True,
    )
    ...
    print(
        f"{_}经过差值后存在空值的行数为：{sm_qielei_all.isnull().T.any().sum()}", "\n"
    )
    ...
    print(
        f"{_}经过众数填充后存在空值的行数为：{sm_qielei_all.isnull().T.any().sum()}",
        "\n",
    )

    sm_qielei_all = sm_qielei_all.round(3)
    sm_qielei_all.to_excel(
        os.path.join(output_path, f"{_}_在全集上的样本.xlsx"),
        index=False,
        sheet_name=f"{_}在全集上的样本",
    )
    # 获取茄类在训练集上的样本
    ...
    sm_qielei.to_excel(
        output_path + f"\{_}_在训练集上的样本.xlsx",
        index=False,
        sheet_name=f"{_}在训练集上的样本",
    )
    ...
    fig2.savefig(
        output_path + f"\{_}_fit_amount_components.svg", dpi=300, bbox_inches="tight"
    )
    ...
    qielei_prophet_amount.to_excel(
        os.path.join(output_path, f"{_}_销量时序分解.xlsx"),
        index=False,
        sheet_name=f"{_}用历史销量计算出的时间效应，合并到训练集中",
    )
    ...
    plt.show()
    fig.savefig(
        output_path + f"\{_}_剔除时间效应前后，{_}销量时序对比.svg",
        dpi=300,
        bbox_inches="tight",
    )
    ...
    sm_qielei_amount_effect_compare.to_excel(
        output_path + f"\{_}_剔除时间效应前后，历史销量的描述性统计信息对比.xlsx",
        sheet_name=f"{_}剔除时间效应前后，历史销量的描述性统计信息对比",
    )
    ...
    plt.show()
    fig.savefig(
        output_path + f"\{_}_数据扩增前后，历史销量的概率密度函数对比图.svg",
        dpi=300,
        bbox_inches="tight",
    )
    ...
    sm_qielei_amt_ext_compare.to_excel(
        output_path + f"\{_}_数据扩增前后，历史销量的描述性统计信息对比.xlsx",
        sheet_name=f"{_}数据扩增前后，历史销量的描述性统计信息对比",
    )
    ...
    comparison_of_distributions_qielei.to_excel(
        output_path + f"\{_}_comparison_of_distributions.xlsx",
        sheet_name=f"{_}_comparison of distributions",
    )
    ...
    all_set.to_excel(
        output_path + f"\{_}_全集上的第一次决策结果及时间效应系数.xlsx",
        index=False,
        encoding="utf-8-sig",
        sheet_name=f"问题2最终结果：{_}全集上的预测订货量及时间效应系数",
    )

    # 使用ref评估最后一周，即预测期的指标
    sm_qielei_all = sm_qielei_all[["busdate", "amount"]].rename(
        columns={"busdate": "销售日期", "amount": "实际销量"}
    )
    ...
    metrics.to_excel(
        output_path + f"\{_}_第一次决策的20种评估指标计算结果.xlsx",
        index=True,
        encoding="utf-8-sig",
        sheet_name=f"{_}20种评估指标的取值",
    )
    print(f"metrics: \n {metrics}", "\n")
    ...
    fig2.savefig(
        output_path + f"\{_}_fit_revenue_components.svg", dpi=300, bbox_inches="tight"
    )
    ...
    forecast_price.to_excel(
        output_path + f"\{_}_销售额时序分解.xlsx",
        index=False,
        encoding="utf-8-sig",
        sheet_name=f"{_}预测销售额",
    )
    ...
    fig2.savefig(
        output_path + f"\{_}_fit_unit_cost_components.svg", dpi=300, bbox_inches="tight"
    )
    forecast_cost.to_excel(
        output_path + f"\{_}_单位成本时序分解.xlsx",
        index=False,
        encoding="utf-8-sig",
        sheet_name=f"{_}预测单位成本",
    )
    ...
    forecast_output.to_excel(
        output_path
        + f"\{_}_在预测期每日的预测销售额、预测单价、预测成本、预测毛利率、加载毛利率时间效应的第二次报童订货量和加载销量时间效应的最终订货量.xlsx",
        index=False,
        encoding="utf-8-sig",
        sheet_name=f"问题2最终结果：{_}在预测期每日的预测销售额、预测单价、预测成本、预测毛利率、加载毛利率时间效应的第二次报童订货量和加载销量时间效应的最终订货量",
    )
    ...
    metrics_new.to_excel(
        output_path + f"\{_}_加载销量时间效应的最终订货量的20种评估指标计算结果.xlsx",
        index=True,
        encoding="utf-8-sig",
        sheet_name=f"{_}加载销量时间效应的最终订货量的评估指标值",
    )
    print(f"metrics_new: \n {metrics_new}", "\n")
    ...
    ax.legend()
    plt.savefig(
        output_path + f"\{_}_qielei_forecast_final.svg", dpi=300, bbox_inches="tight"
    )
    plt.show()
...
if __name__ == "__main__":
    ...
    print(
        f"销售至{account['busdate'].max()}的单品有{sum((account.groupby(['organ', 'code'], as_index=False)['busdate'].max() == account['busdate'].max())['busdate'])}个，占总单品数{len(acct_grup)}之比为：{round(sum((account.groupby(['organ', 'code'], as_index=False)['busdate'].max() == account['busdate'].max())['busdate']) / len(acct_grup) * 100, 2)}%"
    )
    ...
    loss_code = pd.read_excel(
        r"D:\Work info\SCU\MathModeling\2023\data\ZNEW_DESENS\ZNEW_DESENS\sampledata\损耗率.xlsx",
        sheet_name=1,
    )
    ...
    account_commodity = account_commodity.merge(
        loss_code,
        left_on=["code", "name"],
        right_on=["单品编码", "单品名称"],
        how="left",
    )
    account_commodity["loss_rate"] = (
        account_commodity["平均损耗率(%)_单品编码_不同值"] / 100
    )
    account_commodity.drop(
        columns=["单品编码", "单品名称", "平均损耗率(%)_单品编码_不同值"], inplace=True
    )
    pd.set_option("display.max_rows", 20)
    print(
        f"\naccount_commodity\n\nshape: {account_commodity.shape}\n\ndtypes:\n{account_commodity.dtypes}\n\nisnull-columns:\n{account_commodity.isnull().any()}",
    ...
    account_commodity["amount"] = account_commodity["amount"] * (
        1 + account_commodity["loss_rate"]
    )
    account_commodity["sum_cost"] = (
        account_commodity["amount"] * account_commodity["unit_cost"]
    )
    if (
        abs(
            np.mean(
                account_commodity["sum_cost"] / account_commodity["amount"]
                - account_commodity["unit_cost"]
            )
        )
        < 1e-3
    ):
    ...
    Parallel(n_jobs=-1, backend="loky", verbose=50)(
        delayed(ordering_pricing)(
            account_commodity_mean[account_commodity_mean["sm_sort_name"] == _], _
        )
