...
def grouping_heatmap(acct_com, coef, corr_neg):
...
if __name__ == "__main__":
    ...
    codes_between_dates = codes_between_dates[
        codes_between_dates
        == (pd.to_datetime(last_day) - pd.to_datetime(first_day)).days + 1
    ].index
    ...
    codes_between_dates = set(codes_between_dates) - set(
        account_1[account_1["amount"] <= 0]["code"].unique()
    )
    account_1 = account_1[account_1["code"].isin(codes_between_dates)]
    acct_grup_1 = account_1.groupby(["organ", "code"])
    print(
        f"number of commodities after date filtering: {len(acct_grup_1['code'].unique())}",
        "\n",
    )
    # 计算acct_grup_1中每个code的平均销量，按降序排序，取出满足最小陈列量amount_min要求的code集合
    acct_grup_1 = (
        acct_grup_1.mean().sort_values(by=["amount"], ascending=False).reset_index()
    )
    acct_grup_1 = acct_grup_1[acct_grup_1["amount"] >= amount_min]
    print(
        f"number of commodities after amount filtering: {len(acct_grup_1['code'].unique())}",
        "\n",
    )
    ...
    acct_grup = account_mean.sort_values(
        by=["amount*profit"], ascending=False
    ).reset_index()
    # 从code_num_max到code_num_min中随机选择一个数
    code_num = np.random.randint(
        min(code_num_min, acct_grup.shape[0]), min(code_num_max + 1, acct_grup.shape[0])
    )
    ...
    Parallel(n_jobs=-1, verbose=50)(
        delayed(ts_dist)(name, data) for name, data in acct_com.groupby(["name"])
    )
    time.sleep(1)
    print("\nts_dist Parallel finished.", "\n")

    # 相关性分组，画热力图
    groups_ = grouping_heatmap(acct_com, coef, corr_neg)
    ...
    # 对已分组的单品，画出各自的时序图和最优分布图
    Parallel(n_jobs=-1, verbose=50)(
        delayed(grouped_ts_dist)(i, data) for i, data in enumerate(list_df_avg)
    )
    time.sleep(1)
    print("\ngrouped_ts_dist Parallel finished.", "\n")
    ...
    if (
        abs(np.mean(acct_com["sum_cost"] / acct_com["amount"] - acct_com["unit_cost"]))
        < 1e-3
    ):
    ...
    Parallel(n_jobs=-1, backend="loky", verbose=50)(
        delayed(ordering_pricing)(acct_com[acct_com["name"] == _], _)
