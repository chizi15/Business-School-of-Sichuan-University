...
if __name__ == '__main__':
    ...
    # 得到筛选后的各单品的时序图和分布图
    Parallel(n_jobs=-1, verbose=50)(delayed(ts_dist)(name, data) for name, data in acct_com.groupby(['name']))
    time.sleep(1)
    ...
    Parallel(n_jobs=-1, verbose=50)(delayed(grouped_ts_dist)(i, data) for i, data in enumerate(list_df_avg))
    time.sleep(1)
    ...
    Parallel(n_jobs=-1, backend="loky", verbose=50) \
        (delayed(ordering_pricing)(acct_com[acct_com['name'] == _], _) for _ in acct_com['name'].unique())
