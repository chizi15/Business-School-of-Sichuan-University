...
if __name__ == '__main__':
    ...
    if last_day == '2023-06-30':
        ...
        run_com = run_com.groupby(['销售日期', '单品编码', '单品名称', '小分类编码', '小分类名称']).agg({'销量': 'sum', '销售金额': 'sum'}).reset_index()
        run_com['日平均售价'] = run_com['销售金额'] / run_com['销量']
        run_com.dropna(how='any', inplace=True)

        sale_price_cost = run_com.merge(account, left_on=['销售日期', '单品编码'], right_on=['日期', '单品编码'], how='left')
        sale_price_cost.drop(columns=['日期', '课别'], inplace=True)
        

        # 统计品类的日销量、日平均进价、日平均售价
        sale_price_cost['日成本'] = sale_price_cost['销量'] * sale_price_cost['当天进货单价(元)']
        sale_price_cost_sm = sale_price_cost.groupby(['销售日期', '小分类编码', '小分类名称']).agg({'销量': 'sum', '销售金额': 'sum', '日成本': 'sum'}).reset_index()


        # 创建 ExcelWriter 对象
        writer = pd.ExcelWriter(f'{output_path_match}/sale_price_cost.xlsx')
        # 将 sale_price_cost 输出到 '单品' sheet
        sale_price_cost.to_excel(writer, sheet_name='单品', index=False, encoding='utf-8-sig', header=True, float_format='%.2f')
        # 将 sale_price_cost_sm 输出到 '品类' sheet
        sale_price_cost_sm.to_excel(writer, sheet_name='品类', index=False, encoding='utf-8-sig', header=True, float_format='%.2f')
        # 保存 Excel 文件
        writer.save()
