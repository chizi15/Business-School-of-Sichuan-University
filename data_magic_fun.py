...
def clean_with_bizdata(old_path, new_path, new_storage_path, filename, filetp="csv"):
    ...
    if filename != "running":
        if old_df[datecolumn].max() >= new_df[datecolumn].min():
            print(
                ...
                new_df_2[
                    new_df_2.duplicated(
                        subset=["organ", "code", datecolumn], keep=False
                    )
                ],
            )
            new_df_2.drop_duplicates(
                subset=["organ", "code", datecolumn], keep="last", inplace=True
            )
        else:
            print(
                "新表日期最小值大于老表日期最大值，合并后无需去重，原df中存在的重复记录是：",
                new_df_2[
                    new_df_2.duplicated(
                        subset=["organ", "code", datecolumn], keep=False
                    )
        ...
        if old_df[datecolumn].max() >= new_df[datecolumn].min():
            print(
                ...
                new_df_2[
                    new_df_2.duplicated(
                        subset=["organ", "code", datecolumn, "selltime"], keep=False
                    )
                ],
            )
            new_df_2.drop_duplicates(
                subset=["organ", "code", datecolumn, "selltime"],
                keep="last",
                inplace=True,
            )
        else:
    ...
    print(
        f"合并后df行数减去原来两个df行数之和: {new_df_2.shape[0] - old_df.shape[0] - new_df.shape[0]}"
    )
    print(f"clean {filename} ok!", "\n")


def clean_with_commodity(
    old_path, new_path, new_storage_path, filename="commodity", filetp="csv"
):
    ...
    try:
        ...
        new_df.rename(
            columns={
                ...
                "coe": "code",
                "sml_sort": "sm_sort",
                "smlsort_name": "sm_sort_name",
                "mi_sort": "md_sort",
                "miort_name": "md_sort_name",
                "big_sort": "bg_sort",
                "bigsort_name": "bg_sort_name",
            },
            inplace=True,
        )
    ...
    print(
        f"合并去重后df行数减去原来两个df行数之和：{new_df_2.shape[0] - old_df.shape[0] - new_df.shape[0]}"
    )
    print(f"clean {filename} ok!", "\n")
...
def cut_desens_bizdata(storage_path, desens_path, filename, ratio, filetp="csv"):
    ...
    if filename == "account":
        assert set(["amount", "sum_cost", "sum_price", "sum_disc"]) < set(
            df.columns
        ...
        assert set(["amount", "sum_sell", "sum_disc"]) < set(
            df.columns
        ), f"{filename} column error"
        ...
        assert set(
            ["code", "order_pred", "order_real", "loss_real", "loss_theory"]
        ) < set(df.columns), f"{filename} column error"
...
if __name__ == "__main__":
    ...
    output_path = (
        "D:\Work info\WestUnion\data\processed\HLJ\脱敏及筛选后样本数据\output"
    )
    ...
    # 资料表合并
    print("******** 开始合并资料表 ********")
    clean_with_commodity(
        "D:\Work info\WestUnion\data\origin\HLJ",
        "D:\Work info\WestUnion\data\origin\HLJ销售数据20221101-20230420",
        "D:\Work info\WestUnion\data\processed\HLJ\合并后数据",
        filename="commodity",
    )

    clean_with_commodity(
        "D:\Work info\WestUnion\data\processed\HLJ\合并后数据",
        "D:\Work info\WestUnion\data\origin\HLJ销售数据20230421-20230701",
        "D:\Work info\WestUnion\data\processed\HLJ\合并后数据",
        filename="commodity",
    )
