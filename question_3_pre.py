...
# 生成teachers_use_data
running = pd.read_csv(
    r"D:\Work info\SCU\MathModeling\2023\data\ZNEW_DESENS\ZNEW_DESENS\sampledata\running.csv"
)
...
os.makedirs(
    r"D:\Work info\SCU\MathModeling\2023\data\processed\question_3\teachers_use\data",
    exist_ok=True,
)
run_apple.to_csv(
    r"D:\Work info\SCU\MathModeling\2023\data\processed\question_3\teachers_use\data\run_apple_after_processed.csv",
    index=False,
)
...
account_apple["busdate"] = pd.to_datetime(
    account_apple["busdate"], infer_datetime_format=True
)
account_apple.drop(columns=["sum_disc"], inplace=True)
account_apple.to_excel(
    r"D:\Work info\SCU\MathModeling\2023\data\processed\question_3\teachers_use\data\account_apple_after_processed.xlsx",
    index=False,
    sheet_name="经过缺货填充的账表_苹果",
)
...
run_apple_stds = run_apple[
    ~(
        (~run_apple["busdate"].isin(dates_to_keep))
        & (run_apple["selltime"] > "20:00:00")
    )
]
...
os.makedirs(
    r"D:\Work info\SCU\MathModeling\2023\data\processed\question_3\students_use_data",
    exist_ok=True,
)
run_apple_stds.to_csv(
    r"D:\Work info\SCU\MathModeling\2023\data\processed\question_3\students_use_data\run_apple_unprocessed.csv",
    index=False,
)

run_apple_sum = (
    run_apple.groupby(["busdate", "code"])[["amount", "sum_price"]].sum().reset_index()
)
account_apple_run = (
    run_apple_sum.groupby(["busdate"])[["amount", "sum_price"]].mean().reset_index()
)
...
account_apple_stds.to_excel(
    r"D:\Work info\SCU\MathModeling\2023\data\processed\question_3\students_use_data\account_apple_unprocessed.xlsx",
    index=False,
    sheet_name="未经过缺货填充的账表_苹果",
)
