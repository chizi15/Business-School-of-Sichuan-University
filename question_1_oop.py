...
def ts_dist_sm(name, data):
    ...
    plt.title(f'{name}')
    plt.show()
    fig.savefig(output_path + "小分类_%s_销量时序.svg" % name)  # 按小分类聚合后的平均销量和平均价格
    fig.clear()
    ...
    comparison_of_distributions_qielei.to_excel(output_path + f"小分类_{name}_comparison_of_distributions.xlsx", sheet_name=f'{name}_comparison of distributions')
...
def grouping_heatmap(sale_sm, coef, corr_neg):
    ...
    plt.savefig(output_path + "小分类_销量与价格负相关性强的一组中，各个小分类销量间的corr_heatmap.svg")  # 按小分类聚合后的平均销量和平均价格
...
def grouped_ts_dist_sm(i, data, groups_):
    ...
    plt.show()
    fig.savefig(output_path + f"小分类_{str(groups_[i]).replace('[', '(').replace(']', ')')}_按相关性分组合并后的小分类销量时序.svg")  # 按小分类聚合后的平均销量
    fig.clear()
    ...
    comparison_of_distributions_qielei.to_excel(output_path + f"小分类_{groups_[i]}_comparison_of_distributions.xlsx", sheet_name=f'{groups_[i]}_comparison of distributions')
...
def ts_dist_code(name, data):
    ...
    plt.title(f'{name}')
    plt.show()
    fig.savefig(output_path + "单品_%s_销量时序.svg" % name)  # 按小分类聚合后的平均销量和平均价格
    fig.clear()
...
if __name__ == '__main__':
    ...
    Parallel(n_jobs=-1, verbose=50)(delayed(ts_dist_sm)(name, data) for name, data in sale_sm.groupby(['sm_sort_name']))
    ...
    Parallel(n_jobs=-1, verbose=50)(delayed(grouped_ts_dist_sm)(i, data, groups_) for i, data in enumerate(list_df_avg))
    ...
    code_busdate = df.groupby('code').agg(min_busdate=('busdate', 'min'), max_busdate=('busdate', 'max')).reset_index()
    code_busdate_codes = code_busdate[(code_busdate['min_busdate'] == first_day) & (code_busdate['max_busdate'] == last_day)]['code']
    ...
    if sale_sm['code'].dtype == 'float64' or sale_sm['code'].astype(str).str.contains('\.').any():
    ...
    Parallel(n_jobs=-1, verbose=50)(delayed(ts_dist_code)(name, data) for name, data in sale_sm.groupby(['name']))
    ...
    Parallel(n_jobs=-1, verbose=50)(delayed(grouped_ts_dist_code)(i, data, groups_) for i, data in enumerate(list_df_avg))
