...
def prophet_model(
    ...
    if daily:
        m.add_seasonality(
            ...
            prior_scale=daily_prior_scale,
        )
    if hourly:
        m.add_seasonality(
            name="hourly",
            period=hourly_period,
            fourier_order=hourly_fourier_order,
            prior_scale=hourly_prior_scale,
        )
    if minutely:
        m.add_seasonality(
            name="minutely",
            period=minutely_period,
            fourier_order=minutely_fourier_order,
            prior_scale=minutely_prior_scale,
        )
    if secondly:
        m.add_seasonality(
            name="secondly",
            period=secondly_period,
            fourier_order=secondly_fourier_order,
            prior_scale=secondly_prior_scale,
        )
    m.fit(df)
...
# 平稳随机序列检验
def sationary_random_sequence_test(
    data: pd.DataFrame,
    name: list,
    name_type: str,
    y_name: str,
    ...
    for i in range(len(name)):
        ...
        if seasonality_mode == "multiplicative":
            ...
            df["y_no_effect"] = (
                df["y"] / df["multiplicative_terms"] - df["trend"] + df["trend"].mean()
            )
        else:
            effect = forecast[["ds", "trend", "additive_terms"]]
            df = df.merge(effect, on=["ds"], how="left")
            df["y_no_effect"] = (
                df["y"] - df["additive_terms"] - df["trend"] + df["trend"].mean()
            )
        ...
        # 找到离群值
        outliers = df[
            (df["y_no_effect"] < (q1 - 1.5 * iqr))
            | (df["y_no_effect"] > (q3 + 1.5 * iqr))
        ]
        ...
        match name_type:
            case "sm_sort_name":
                plt.savefig(
                    paths[4]
                    + f"{name[i]}_去掉时间类效应的{y_name}的自相关图和偏自相关图.svg",
                    format="svg",
                )
            case "name":
                plt.savefig(
        ...
        match name_type:
            ...
            case "name":
                with open(
                    paths[6 + 1] + f"{name[i]}_{y_name}_adf_result.txt", "w"
                ) as f:
                    f.write(description + adf_result_str)
            case _:
                raise ValueError(
                    f"Unexpected name_type: {name_type} or y_name: {y_name}"
                )
...
if __name__ == "__main__":
    ...
    # 数据预处理
    code_sm = pd.read_excel(
        f"{input_path}" + "附件1-单品-分类.xlsx", sheet_name="Sheet1"
    )
    ...
    cost_code = pd.read_excel(
        f"{input_path}" + "附件3-进价-单品.xlsx", sheet_name="Sheet1"
    )
    loss_code = pd.read_excel(
        f"{input_path}" + "附件4-损耗-单品.xlsx", sheet_name="Sheet1"
    )
    ...
    acct_code = (
        run_code.groupby(["code", "busdate"])[["amount", "sum_price"]]
        .sum()
        .reset_index()
    )
    ...
    print(
        f"sale_sm['sm_sort_name'].nunique(): {sale_sm['sm_sort_name'].nunique()}\nsale_code['name'].nunique(): {sale_code['name'].nunique()}\n"
    )
